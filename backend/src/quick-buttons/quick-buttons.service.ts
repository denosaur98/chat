...
export class QuickButtonsService {
  ...
  getButtonsByMessenger(messengerType: string) {
    return {
      limit: this.buttonsMap[messengerType]?.limit,
      buttonItems: this.buttonsMap[messengerType]?.buttonItems.map(({ id, text }) => ({ id, text })) || []
    };
  }

  getButtonResponse(messengerType: string, buttonId: number) {
    const button = this.buttonsMap[messengerType]?.buttonItems.find(b => b.id === buttonId);
